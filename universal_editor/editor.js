...
function gp_init_inline_edit(area_id, section_object){
  ...
  gp_editor.SaveData = function(){
    ...
    var attributes = gp_editor.setSectionAttributes() ? '&' + $.param( { attributes : section_object.attributes }) : '';
    var content = encodeURIComponent( gp_editor.edit_section.html() );
    return 'gpcontent=' + content + '&' + values + attributes;
  ...
  gp_editor.setSectionAttributes = function(){
    var inputs_applyto_classes = gp_editor.ui.controls
      .find("input[data-applyto='class']:not(.editor-ctl-no-submit, [type='checkbox'], [type='radio']), select[data-applyto='class'], textarea[data-applyto='class']");
    ...
    // collect and concatenate all possible classes from radio btns or select->options
    inputs_applyto_classes.each( function(){
      if( $(this).closest(".editor-ctl-radio-group").length || $(this).closest(".editor-ctl-select").length ){
        // radio group or select
        $(this).closest(".editor-ctl-box").find('input[type="radio"], option').each( function(){
          var classes = $(this).attr("value") ? $(this).attr("value") : $(this).text();
          possible_classes += classes + ' ';
        });
      }
      selected_classes += $(this).val() + ' ';
    });

    //console.log("current_classes = " + current_classes);
    //console.log("possible_classes = " + possible_classes);
    //console.log("selected_classes = " + selected_classes);

    var tmp_div = $("<div>");
    tmp_div
      .addClass(current_classes)
      .removeClass(possible_classes)
      .addClass(selected_classes);
    var new_classes = tmp_div.attr("class") ? tmp_div.attr("class") : '';
    ...
    // console.log("section_object.attributes = ", section_object.attributes);
    return (!$.isEmptyObject(section_object.attributes) ); // returns true if attribtes exist, else false
    
  }; // gp_editor.setSectionAttributes --end
  ...
  gp_editor.updateSection = function(){
    ...
    gp_editor.setSectionAttributes();

    var data = {
      cmd         : 'save_custom_section',
      type        : section_object.type,
      attributes  : section_object.attributes
    };
  ...
  gp_editor.setFile = function(fileUrl, input_selector){
    ...
    if( input_selector instanceof jQuery ){
      // input_selector is a jQuery object
      console.log("setFile: input_selector = jQuery obj");
      input_selector.val(fileUrl).trigger("change");
    }else{
      // input selector is a string
  ...
  gp_editor.addBunchControl = function(sub_controls){
    ...
    $.each(sub_controls, function(a,map){
      ...
      new_bunch += '<input data-item="' + bunch_name + i + map.sub_name + '-bunch_box" '
        ...
        + 'data-control=\'' + sub_control_data + '\' '
        + 'type="hidden" name="values[' + bunch_name + '][' + i + '][' + map.sub_name + ']" '
        + 'value="' + map.value + '"/>';
  ...
  gp_editor.controls = {
    ...
    radio_group : function(control_map){
      ...
      radio_group += '<input id="editor-ctl-' + item + '" type="hidden" name="values[' + item + ']" value="' + value + '"' + attributes + '/>';
      radio_group += '</div>';
